...
export_to_data_csv <- function(df, filename = NULL) {
  # If no filename is provided, use the deparsed name of the dataframe object
  if (is.null(filename)) {
    # Capture the name of the object passed to the function
  ...
  if (!dir.exists(data_dir)) {
    ...
    cat("Created directory:", fs::path_abs(data_dir), "\n")
  }

  # Combine directory and filename into a full path
  full_path <- fs::path(data_dir, filename)

  # Write the dataframe using write_csv from the readr package
  write_csv(df, full_path)

  cat("Successfully exported '", deparse(substitute(df)), "' to:\n", fs::path_abs(full_path), "\n", sep="")
}
